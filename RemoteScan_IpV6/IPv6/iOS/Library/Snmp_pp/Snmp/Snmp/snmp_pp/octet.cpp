...
int operator<(const OctetStr &lhs, const OctetStr &rhs)
{
  SmiUINT32 maxlen = lhs.smival.value.string.len > rhs.smival.value.string.len
...
{
  OctetStr to(rhs);
  SmiUINT32 maxlen = lhs.smival.value.string.len > to.smival.value.string.len
...
const char *OctetStr::get_printable() const
{
  ...
  if (output_buffer_len < smival.value.string.len + 1)
  {
    ...
    ncthis->output_buffer_len = (unsigned int)smival.value.string.len + 1;
  }
...
OctetStr OctetStr::from_hex_string(const OctetStr &hex_string)
{
  ...
  for (p = (unsigned int)hex_string.len(); p > 0; p--)
  {
    unsigned char c = *ptr++;
    if (c != ' ')
    {
...
int OctetStr::get_asn1_length() const
{
  if (smival.value.string.len < 0x80)
    return (int)smival.value.string.len + 2;
  else if (smival.value.string.len < 0x100)
    return (int)smival.value.string.len + 3;
  else if (smival.value.string.len < 0x10000)
    return (int)smival.value.string.len + 4;
  else if (smival.value.string.len < 0x1000000)
    return (int)smival.value.string.len + 5;
  return (int)smival.value.string.len + 6; // should be safe for some time...
}
