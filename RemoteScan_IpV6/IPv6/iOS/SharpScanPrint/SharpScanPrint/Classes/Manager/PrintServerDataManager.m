...
@implementation PrintServerDataManager
...
{
    // DATファイル読込み
    NSString *pstrFileName = [self.BaseDir stringByAppendingString:S_PRINTSERVERDATA_DAT];
    ...
    for (NSInteger nIndex = 0; nIndex < [obj count]; nIndex++)
    {
        ...
        // プリントサーバー情報をDATファイルから取得
        printerData = [obj objectAtIndex:nIndex];
        // プリントサーバー情報をPrinterDataクラスに追加
...
- (NSMutableArray*)GetIndexListInclude
{
    ...
    for (NSInteger nIndex = 0; nIndex < [obj count]; nIndex++)
    {
        ...
        if(!printerData.ExclusionList)
        {
            // 除外されていないプリンタのリストのIndexがプリントサーバー情報の何番目のIndexであるか取得
...
- (NSMutableArray*)GetIndexListExclude
{
    ...
    for (NSInteger nIndex = 0; nIndex < [obj count]; nIndex++)
    {
        ...
        if(printerData.ExclusionList)
        {
            // 除外リストのIndexがプリントサーバー情報の何番目のIndexであるか取得
            [parrTempData addObject:[NSNumber numberWithInteger:nIndex]];
...
// 選択中MFPのIndex保持 プリントサーバー用
- (void)SetDefaultMFPIndex2:(NSString*) pstrKey PrimaryKeyForCurrrentWifi:(NSString*) pstrKeyForCurrentWiFi
{
    // PROFILE情報の取得
    ProfileDataManager* profileDataManager = [[ProfileDataManager alloc] init];
    ProfileData *profileData = nil;
