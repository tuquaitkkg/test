...
@implementation HttpLegacyManeger
...
@synthesize		addDic;				// post時のキー情報
@synthesize		updateDic;          // post時のキー情報
@synthesize		deleteDic;          // post時のキー情報
...
- (void)makePostData:(NSMutableURLRequest *)request
{
	@autoreleasepool
    {
        ...
        // ggt_hidden(1)
        if ( postMode == HTTP_POST_ADD_LEGACY )
        {
            // 新規
//            if ( userAuthen == HTTP_USERA_INVALID_LEGACY )
//            {
//                // ユーザ認証：無効
////                str  = [NSString stringWithFormat:@"http://%@/%@", self.mfpipAder, POST_ADD_HTML];
//                str  = [NSString stringWithFormat:@"http://%@/%@", self.mfpipAder, [addDic objectForKey:@"url"]];
//                [request setURL:[NSURL URLWithString:str]];
//                // HTTP メソッドを指定
//                [request setHTTPMethod:[addDic objectForKey:@"action"]];
//            }
//            else
//            {
                // ユーザ認証：有効
...
//            str  = [NSString stringWithFormat:@"http://%@/%@\?profid=4", self.mfpipAder, POST_MOD_NST];
            str  = [NSString stringWithFormat:@"http://%@/%@\?profid=4", self.mfpipAder, [updateDic objectForKey:@"url"]];
            [request setURL:[NSURL URLWithString:str]];
            // HTTP メソッドを指定
            [request setHTTPMethod:[updateDic objectForKey:@"action"]];
       }
...
//        str     = [NSString stringWithFormat:@"ggt_textbox(4)=%@", self.myname];
        str     = [NSString stringWithFormat:@"%@=%@", [addDic objectForKey:@"PROFILENAME"], self.myname];
        encode	= nil;
        encode	= ((NSString*)CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(
                                                                                        kCFAllocatorDefault,
                                                                                        (CFStringRef)str,
...
//        str		= [NSString stringWithFormat:@"ggt_textbox(5)=%@", self.serchStr];
        str		= [NSString stringWithFormat:@"%@=%@", [addDic objectForKey:@"PROFILESORT"], self.serchStr];
        encode	= ((NSString*)CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(
                                                                                        kCFAllocatorDefault,
                                                                                        (CFStringRef)str,
                                                                                        NULL,
        ...
        // プロファイルパス　値は空で追加
        str  = [NSString stringWithFormat:@"%@=&",[addDic objectForKey:@"PROFILEPATH"]];
        [body appendData:[str dataUsingEncoding:NSUTF8StringEncoding]];
...
//        [body appendData:[str dataUsingEncoding:NSUTF8StringEncoding]];
        ...
        // *************
//        str  = @"ggt_textbox(9)=";
        str  = [NSString stringWithFormat:@"%@=",[addDic objectForKey:@"HOSTIP"]];
        str	 = [[str stringByAppendingString:self.ipAder]stringByAppendingString:@"&"];
        [body appendData:[str dataUsingEncoding:NSUTF8StringEncoding]];
        
        // ポート番号（必須）		: ggt_textbox(10) 4687固定
//        str  = @"ggt_textbox(10)=";
        str  = [NSString stringWithFormat:@"%@=",[addDic objectForKey:@"HOSTPORT"]];
...
//        }
        ...
        // *************
//        str  = @"ggt_textbox(16)=";
        str  = [NSString stringWithFormat:@"%@=",[addDic objectForKey:@"ftp:USER"]];
        ...
        // *************
//        str  = @"ggt_textbox(17)=";
        str  = [NSString stringWithFormat:@"%@=",[addDic objectForKey:@"ftp:PASSWORD"]];
...
//        // *************
//        unsigned int len = [self.password length];
//        if (len > 0 )
//        {
...
//        str  = @"action=desksubmitbtn&";
        str  = [NSString stringWithFormat:@"action=%@&",[addDic objectForKey:@"action"]];
        [body appendData:[str dataUsingEncoding:NSUTF8StringEncoding]];
...
//        }
        ...
        NSString *strr= [[NSString alloc] initWithData:body encoding:NSUTF8StringEncoding];
        DLog(@"%@",strr);
        //
        //	内容物追加
        //
...
//        str  = [NSString stringWithFormat:@"http://%@/%@", self.mfpipAder, POST_DEL_NST];
        str  = [NSString stringWithFormat:@"http://%@/%@", self.mfpipAder, [deleteDic objectForKey:@"url"]];
        [request setURL:[NSURL URLWithString:str]];
        [request setHTTPMethod:@"POST"];
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response
{
	...
	@autoreleasepool
    {
        @try
        {
            ...
            // レスポンスコードの判定
            //
            DLog(@"statusCode:%ld",(long)[(NSHTTPURLResponse *)response statusCode])
            ...
            extendSharpSettingStatus = [[[(NSHTTPURLResponse *)response allHeaderFields] objectForKey:@"Extend-sharp-setting-status"] intValue];
            if (postMode == HTTP_POST_ADD_LEGACY) { //***確認用
                extendSharpSettingStatus = 949;
            }
            DLog(@"Extend-sharp-setting-status:%d", extendSharpSettingStatus);
            ...
            if( disconnect )
            {
                ...
                switch(postMode)
                {
                    case HTTP_POST_ADD_LEGACY:
                    case HTTP_POST_MOD_LEGACY:
                        //
                        // 登録 or 更新
                        //
//                        if ( userAuthen == HTTP_USERA_INVALID_LEGACY )
...
//                            error = [NSString stringWithFormat:@"%d:%@\r\n%@",
//                                     [(NSHTTPURLResponse *)response statusCode],
//                                     self.errMsg, [addDic objectForKey:@"url"]];
//                        }
//                        else
//                        {
                            // ユーザ認証：有効
...
//                                 self.errMsg, POST_DEL_NST];
                        error = [NSString stringWithFormat:@"%zd:%@\r\n%@",
                                 [(NSHTTPURLResponse *)response statusCode],
                                 self.errMsg, [deleteDic objectForKey:@"url"]];
                        break;
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
	...
	@autoreleasepool
    {
        ...
        switch (extendSharpSettingStatus) {
            case 0:
                rtn = 1;
                break;
            case 949:
        ...
        switch(postMode)
        {
            case HTTP_POST_ADD_LEGACY:		// プロファイル登録
                //
                // 登録(1) 結果判定
                //
//                rtn = [self responceDataJudge_add];
            ...
            case HTTP_POST_MOD_LEGACY:		// プロファイル更新
                //
                // 更新(2) 結果判定
                //
//                rtn = [self responceDataJudge_mod];
            ...
            case HTTP_POST_DEL_LEGACY:		// プロファイル削除
                //
                // 削除(3) 結果判定
                //
