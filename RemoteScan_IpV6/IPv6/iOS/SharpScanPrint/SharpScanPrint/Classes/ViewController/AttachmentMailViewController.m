...
@interface AttachmentMailViewController ()
@property (nonatomic,assign) BOOL isFirstTap; // セルをタップ済みかどうか(連打不可対応)
@end

@implementation AttachmentMailViewController

@synthesize delegate;							// ファイルパス
...
- (void) changeCheckbox:(NSIndexPath *)indexPath {
    ...
    // 選択セルのチェック
    // 選択されたセルのindexPathをmArrayに溜め込んでいく。
...
- (void)MoveView:(NSIndexPath *)nIndexPath
{
    ...
    if([scanData isDirectory ])
    {
        ...
        AttachmentMailViewController* selectView = [[AttachmentMailViewController alloc] init];
        //ZIPファイルの場合
...
- (void)moveMultiPrintPictView
{
    ...
    //selectFilePathArrayのリセット
    [self.selectFilePathArray removeAllObjects];
    ...
    DLog(@"self.selectFilePathArray= %@",self.selectFilePathArray);
