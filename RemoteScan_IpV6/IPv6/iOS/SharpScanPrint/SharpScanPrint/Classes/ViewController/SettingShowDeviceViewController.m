...
@implementation SettingShowDeviceViewController
...
- (void)viewDidLoad
{
    ...
    // ナビゲーションバー
    // タイトル設定
    // self.navigationItem.title = [m_pPrinterInfo PrinterName];
        ...
        if (cell == nil)
        {
            ...
            switch(indexPath.row){
                ...
                case 1:
                {
                    ...
                    //表示名の文字数調整（半角は32文字以内、全角は16文字以内)
                    NSString*tmpProfileName = [CommonUtil trimString:profileData.profileName halfCharNumber:32];
...
-(void)rsManagerDidFailWithError:(RSHttpCommunicationManager*)manager
{
    if([manager isKindOfClass:[RSmfpifManager class]])
    {
        ...
        m_pPrinterInfo.isCapableNetScan = YES;
        m_pPrinterInfo.isCapablePrintRelease = NO;
        m_pPrinterInfo.enabledDataReceive = NO;
    } else if([manager isKindOfClass:[RSmfpifServiceManager class]])
    {
        m_pPrinterInfo.IsCapableRemoteScan = NO;
        m_pPrinterInfo.IsCapableNovaLight = [mfpManager isCapableNovaLight];
