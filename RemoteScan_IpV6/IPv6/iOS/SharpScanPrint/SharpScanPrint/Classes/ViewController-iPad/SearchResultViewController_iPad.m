...
@interface SearchResultViewController_iPad () <AdvancedSearchResultViewController_iPadDelegate>
...
@implementation SearchResultViewController_iPad
...
- (void)viewDidLoad
{
    DLog(@"viewDidLoad     SearchResultViewController_iPad");
    [super viewDidLoad];
...
{
    DLog(@"viewWillAppear     SearchResultViewController_iPad");
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    // 現在検索しているキーワードを他の検索結果画面に設定する
    [self settingSearchKeywordToAllSearchResultView];
...
- (void)MoveView:(NSIndexPath *)nIndexPath
{
    ...
    if([[m_pScanMgr loadScanDataAtIndexPath:nIndexPath] isDirectory ])
    {
        ...
        {
            SelectFileViewController_iPad* selectView;
            selectView = [[SelectFileViewController_iPad alloc] init];
...
-(void)ChangeDetailView:(UIViewController*)pViewController didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(![leftViewClassName isEqual:[self description]])
    {
        ...
        [pRootViewController updateView:self.PrevViewID searchPreViewID:SearchResultTypeView didSelectRowAtIndexPath:indexPath scrollOffset:[self.tableView contentOffset]];
    ...
    else
    {
        ...
        if ( [[[pDetailNavController viewControllers] lastObject] isKindOfClass: [ArrangePictViewController_iPad class]] ) {
            // 画面を取り除く
...
- (void)SetHeaderView
{
    if (!self.tableView.tableHeaderView) {
        ...
        else
        {
            ...
            if([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0)
            {   // iOS7以上なら
                m_searchController.hidesNavigationBarDuringPresentation = YES;
                m_searchController.searchBar.searchBarStyle = UISearchBarStyleMinimal;
            }
...
{
    DLog(@"searchBarSearchButtonClicked SearchResultViewController_iPad");
    // iPad用
    // キーボードを非表示
...
{
    DLog(@"searchBarCancelButtonClicked SearchResultViewController_iPad");
    sortTypeButton.hidden = NO;
...
-(void)willDisAppearAdvancedSearchResultViewController:(AdvancedSearchResultViewController_iPad *)advancedSearchResultVC
{
    ...
    //searchボタンが押されたことにする
    [self searchBarSearchButtonClicked:m_searchController.searchBar];
...
-(void)advancedSearchRightBarButtonClicked
{
    ...
    AdvancedSearchResultViewController_iPad* pAdvancedSearchResultViewController_iPad;
    pAdvancedSearchResultViewController_iPad = [[AdvancedSearchResultViewController_iPad alloc]init];
    pAdvancedSearchResultViewController_iPad.scanDirectory = self.scanDirectory;

    pAdvancedSearchResultViewController_iPad.bCanDelete = self.bCanDelete;
    pAdvancedSearchResultViewController_iPad.pstrSearchKeyword = self.pstrSearchKeyword;
    pAdvancedSearchResultViewController_iPad.PrevViewID = self.PrevViewID;
    pAdvancedSearchResultViewController_iPad.bSubFolder = self.bSubFolder;
    pAdvancedSearchResultViewController_iPad.bFillterFolder = self.bFillterFolder;
    pAdvancedSearchResultViewController_iPad.bFillterPdf = self.bFillterPdf;
    pAdvancedSearchResultViewController_iPad.bFillterTiff = self.bFillterTiff;
    pAdvancedSearchResultViewController_iPad.bFillterImage = self.bFillterImage;
    pAdvancedSearchResultViewController_iPad.bFillterOffice = self.bFillterOffice;
    pAdvancedSearchResultViewController_iPad.sessionViewController = self;
    // 複数印刷対応_iPad
    pAdvancedSearchResultViewController_iPad.fromSelectFileVC = self.fromSelectFileVC;
    pAdvancedSearchResultViewController_iPad.PrevViewID = self.PrevViewID;
    ...
    if(![leftViewClassName isEqual:[self description]])
    {
        pAdvancedSearchResultViewController_iPad.bSetTitle = YES;
    }
    
    if( [[self.navigationController.viewControllers lastObject] isMemberOfClass:[AdvancedSearchResultViewController_iPad class] ])
    {//既に詳細検索結果が表示されている場合
        //一旦戻る
    ...
    //デリゲート設定
    pAdvancedSearchResultViewController_iPad.delegate = self;
    
    [self.navigationController pushViewController:pAdvancedSearchResultViewController_iPad animated:YES];
}
...
// 現在検索しているキーワードを他の検索結果画面に設定する
- (void)settingSearchKeywordToAllSearchResultView
{
    SharpScanPrintAppDelegate* pAppDelegate = (SharpScanPrintAppDelegate*)[[UIApplication sharedApplication] delegate];
    NSArray * splitViewControllers = pAppDelegate.splitViewController.viewControllers;
    
    if ( ! splitViewControllers ) {
        // 処理しない
        return;
    }
    
    // スプリットビューコントローラー抽出
    for (UIViewController * viewController in splitViewControllers) {
        ...
        DLog(@"[navigationController viewControllers] %@", [navigationController viewControllers]);
        
        // スプリットビュー内部のナビゲーションビューコントローラーを抽出
        // 逆順に検索する（今の階層から遡って検索する。ルートから検索しない）
        for (UIViewController * viewControllerInNavi in [[navigationController viewControllers] reverseObjectEnumerator]) {
            
            // 直前の画面がこのクラスと同じではない場合
            if ( ! [viewControllerInNavi isKindOfClass: [self class]] ) {
                
                // 詳細画面以外の場合
                if ( ! [viewControllerInNavi isKindOfClass: [PrintPictViewController_iPad class]]) {
                    // 処理しない
                    break;
                }
                // 詳細画面の場合
            ...
            DLog(@"Setting Search Keyword %@", self.pstrSearchKeyword);
            
            SearchResultViewController_iPad * searchResultViewController = (SearchResultViewController_iPad *)viewControllerInNavi;
            
            // 現在検索しているキーワードを他の検索結果画面に設定する
            searchResultViewController.pstrSearchKeyword = self.pstrSearchKeyword;
            
            // 直前の画面にだけ設定するので処理を終える
            break;
        }
    }
}
...
- (void)shareAction:(id)sender {
    ...
    __block SearchResultViewController_iPad *block_self = self;
