...
@interface WebPagePrintViewController_iPad ()
...
@implementation WebPagePrintViewController_iPad
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = S_PRINT_SEL_WEBPAGE;
...
//    m_webView.scalesPageToFit = YES;
    ...
    m_urlTextField.autoresizingMask = UIViewAutoresizingFlexibleWidth;
...
{
    // アラートが出ているなら消しておく
    [self.ex_alert dismissViewControllerAnimated:YES completion:nil];
...
- (void)webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error {
    ...
    if ([error code] != NSURLErrorCancelled) {
        ...
        [[NSOperationQueue mainQueue] addOperationWithBlock:^(){
            self.ex_alert = [self makeTmpExAlert:nil message:[error localizedDescription] cancelBtnTitle:nil okBtnTitle:MSG_BUTTON_OK tag:0 showFlg:YES];
        }];
...
- (void)showAlertForPrintError {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^(){
        ...
        // ★TODO:他言語化
        self.ex_alert = [self makeTmpExAlert:nil message:MSG_CANNOT_PRINT_WEB_PAGE cancelBtnTitle:nil okBtnTitle:MSG_BUTTON_OK tag:0 showFlg:YES];
    }];
}
...
- (void)movePrintPictView {
    if(delegate){
        ...
        NSString *path =[self getSavePdfFilePath];
        DLog(@"path:%@",path);
        NSNotification *n = [NSNotification notificationWithName:WP_ENTER_BUTTON_PUSHED2 object:self];
...
- (void)showAlertForConfirmPrint {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^(){
        ...
        // ★TODO:他言語化
        self.ex_alert = [self makeTmpExAlert:nil message:MSG_CONFIRM_PRINT_WEB_PAGE cancelBtnTitle:MSG_BUTTON_CANCEL okBtnTitle:MSG_BUTTON_OK tag:0 showFlg:YES];
    }];
}
...
- (void)printFunction {
    ...
    if (self.delegate == nil) {
        ...
        NSNotification *n = [NSNotification notificationWithName:WP_ENTER_BUTTON_PUSHED1 object:self];
        NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];
        [nc postNotification:n];
    }
