...
@interface RSS_ManuscriptViewController_iPad ()

@end

@implementation RSS_ManuscriptViewController_iPad
...
/**
 * 初期化
 * @param mArray　原稿サイズリスト
 * @param sArray　保存サイズリスト
 * @param delegate デリゲート（親画面）
 */
...
{
    self = [super initWithNibName:@"RSS_ManuscriptViewController_iPad" bundle:nil];
    if (self) {
...
- (void)viewDidLoad
{
    ...
    // テーブルの背景色を設定
    UIView* bgView = [[UIView alloc]initWithFrame:manuscriptTableView.frame];
    [manuscriptTableView setBackgroundView:bgView];
    // スクロールの必要がないのでNO
    //manuscriptTableView.scrollEnabled = NO;
...
//    manuscriptSetImageView.image = pIconImage;
    ...
    //viewWillAppear, viewDidAppaerが呼ばれない（単に親のviewControllerのviewにaddSubViewされているだけだからか？）のでここで呼んでおく。
...
//                                               autorelease];
             ...
             || [[parentVCDelegate.rssViewData.originalSize getSelectValue] isEqualToString:@"japanese_postcard_a6"])
    {
        ...
        {
            // カラーモードボタンのラベルを更新(カスタムサイズの登録から戻った時はこのボタンだけ更新したい
            [parentVCDelegate updateBtnThirdSetting];
        }
...
-(void)setStrOriginalSize:(NSString*)str
{
    ...
    {//原稿サイズが自動の場合
        ...
        if ([m_sections containsObject:[NSNumber numberWithInteger: kSectionKenchiSize]] == YES)
        {//原稿検知サイズセクションが表示中のときは、更新する。
            UITableViewCell * cell = [manuscriptTableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:1]];
            [cell.textLabel setText:strOriginalSize];
        }
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch ( [[m_sections objectAtIndex: indexPath.section] intValue] ) {
        ...
        case kSectionGenkouSize:
            // 原稿サイズ
            cell.textLabel.text = [manuscriptSizeList objectAtIndex:m_nSelectedManuscriptSizeIndexRow];//[parentVCDelegate replaceRSStr:[manuscriptSizeList objectAtIndex:m_nSelectedManuscriptSizeIndexRow]];
        ...
        case kSectionKenchiSize:
            // 現在の検知サイズ
            cell.textLabel.text = strOriginalSize;
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            cell.accessoryType = UITableViewCellAccessoryNone;
            break;
        case kSectionSaveSize:
            // 保存サイズ
            cell.textLabel.text = [saveSizeList objectAtIndex:m_nSelectedSaveSizeIndexRow];//[parentVCDelegate replaceRSStr:[saveSizeList objectAtIndex:m_nSelectedSaveSizeIndexRow]];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            cell.accessoryView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:S_IMAGE_PICKER]];
            break;
        case kSectionCustomSize:
            // カスタムサイズの登録
            cell.textLabel.text = S_MANUSCRIPT_CUSTOMSIZE_REGISTER;
            cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
            break;
        case kSectionCellSize:
            // セルタイトル
//            cell.textLabel.text = [manuscriptSetList objectAtIndex:indexPath.row];//[parentVCDelegate replaceRSStr:[manuscriptSetList objectAtIndex:indexPath.row]];
            // 選択状態
...
- (UIView *) tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    if(sectionNum == kSectionSaveSize){
        ...
        // タイトルラベル作成
        UILabel* titleLbl = [[UILabel alloc]initWithFrame:CGRectMake(40, 0, tableView.bounds.size.width - 120, 32)];
...
-(void)showFormatSettingPickerViewFromCell:(UITableViewCell*)cell
{
    ...
    pickerViewController.m_notificationName = @"Manuscript Value";
    ...
    // popOver表示
    UIPopoverArrowDirection arrowDirect = (m_nSelPicker < 3 ? UIPopoverArrowDirectionUp : UIPopoverArrowDirectionDown);
    if(!m_popOver.popoverVisible)
    {
        ...
        // 通知の監視を開始
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getPickerValueAction:) name:@"Manuscript Value" object:nil];
    }
}
...
- (void)getPickerValueAction:(NSNotification*)notification
{
    ...
    switch (m_nSelPicker) {
        case kSectionGenkouSize:
            // 原稿サイズ
            m_nSelectedManuscriptSizeIndexRow = row;
            [parentVCDelegate.rssViewData.originalSize setSelectValue:row];
            // カスタムサイズ(名刺含む)が選択されている場合
            NSUInteger originalSizeCount = [[parentVCDelegate.rssViewData.originalSize getSelectableArray] count];
            originalSizeCount -= [[parentVCDelegate.rssViewData.customSizeListData getPaperSizeArray] count];
            originalSizeCount -= [[parentVCDelegate.rssViewData.extraSizeListData getPaperSizeArray] count];
            ...
            //保存サイズの表示/非表示設定
            if (m_nSelectedManuscriptSizeIndexRow > originalSizeCount/*[originalSizeData getFullKeyCount]*/ - 1) {
                //保存サイズに自動を設定
                m_nSelectedSaveSizeIndexRow = 0;
                [parentVCDelegate.rssViewData.sendSize setSelectValue:0];
        ...
        case kSectionSaveSize:
            // 保存サイズ
            m_nSelectedSaveSizeIndexRow = row;
            [parentVCDelegate.rssViewData.sendSize setSelectValue:row];
            // ボタンラベルを更新する
            [parentVCDelegate updateSetting];
            break;
...
-(void)setScOrientation
{
    ...
    scOrientation.frame = CGRectMake(30, 0, 420, 80);
