...
@implementation SettingSelDeviceViewController_iPad
...
@synthesize HidesBackButton;// iPad用
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (nSaveIdx == -1)
    {
        ...
        if([pPrinterMgr CountOfPrinterDataInclude] > 0)
        {
            // 一番上のプリンタの情報更新
...
{
    SettingShowDeviceViewController_iPad* pSettingViewController;
    
    if(nIndexPath.section == 0)
    {
        pSettingViewController = [[SettingShowDeviceViewController_iPad alloc] initWithStyle:UITableViewStyleGrouped];
        // プリンタ情報をセット
...
// iPad用
{
    // 中央(横)にボタンが配置されるようx座標を算出
    CGFloat buttonWidth = 627.0;
    //CGFloat btnAutoX = 30;
    CGFloat btnAutoX = 35;
    CGFloat buttonY = 29;
    // iPad用
    ...
    // iPad用
    //pbtnAuto.frame = CGRectMake(0.0, 0.0, 300.0, 50.0);                                 // ボタンサイズ
    //pbtnAuto.frame = CGRectMake(btnAutoX, buttonY, 300.0, 50.0);                        // ボタンサイズ
    pbtnAuto.frame = CGRectMake(btnAutoX, buttonY, 260.0, 50.0);                        // ボタンサイズ
    ...
    //pbtnAuto.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
    pbtnAuto.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
    
	// 手動追加ボタン生成
	UIButton* pbtnManual = [UIButton buttonWithType:UIButtonTypeCustom];                               // ボタンタイプ
    [pbtnManual setExclusiveTouch: YES];
    // iPad用
    //pbtnManual.frame = CGRectMake(320.0, 0.0, 300.0, 50.0);                              // ボタンサイズ
    //pbtnManual.frame = CGRectMake(btnAutoX + 257.0, buttonY, 300.0, 50.0);              // ボタンサイズ
    pbtnManual.frame = CGRectMake(btnAutoX + 293.0, buttonY, 260.0, 50.0);              // ボタンサイズ
    // 背景画像設定
    ...
    //pbtnManual.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin;
    pbtnManual.autoresizingMask = UIViewAutoresizingFlexibleWidth |UIViewAutoresizingFlexibleLeftMargin;
    
    // iPad用
    /*
     //フッターにボタン設定
     UIView *footerView = [[UIView alloc] initWithFrame:CGRectMake(5.0, 0.0, 300.0, 100.0)];
     UIView *footerView = [[UIView alloc] initWithFrame:CGRectMake(buttonX, 0.0, buttonWidth, 100.0)];
     ...
     */
	UIView *headerView = [[UIView alloc] initWithFrame:CGRectMake(0.0, 0.0, buttonWidth, 90.0)];
...
- (void)EndResolveAddress
{
    ...
    for (NSInteger nIndex = 0; nIndex < [self.Services count]; nIndex++)
    {
        ...
        // mfpif.xml取得
        //--------------------
        NSURL* url = [NSURL URLWithString:[NSString stringWithFormat:@"http://%@:%d/", [CommonUtil optIPAddrForComm:resolveIPAddr], 80]];   // resolveIPAddrは自端末のIPアドレス形式と合っている。
        ...
        if(newData.IsCapableRemoteScan || newData.isCapablePrintRelease){
            ...
            url = [NSURL URLWithString: [NSString stringWithFormat:@"http://%@:%d/", [CommonUtil optIPAddrForComm:resolveIPAddr], 80]]; // resolveIPAddrは自端末のIPアドレス形式と合っている。
...
- (void)popRootView
{
    UIViewController* detailViewController = [self.navigationController.viewControllers objectAtIndex:0];
    
    if([detailViewController isKindOfClass:[SettingSelInfoViewController_iPad class]])
    {
        // 設定情報選択画面のメニューボタン表示フラグをONに設定
