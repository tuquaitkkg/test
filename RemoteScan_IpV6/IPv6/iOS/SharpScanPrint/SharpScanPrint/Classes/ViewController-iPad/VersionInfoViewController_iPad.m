...
@implementation VersionInfoViewController_iPad
...
- (void)viewDidLoad
{
    ...
    {   
        ...
        m_btnSpecialMode.titleLabel.font = [UIFont systemFontOfSize:22];                            // ボタンのフォントサイズ
                          ...
                          :(UILabel *)txtLabel
{
    ...
    txtLabel.translatesAutoresizingMaskIntoConstraints = YES;   // autolayoutでsetFrameを使う。
...
- (void)makePdfPrintRangeExpansionButton{
    ...
    CGFloat startY = [S_LANG isEqualToString:S_LANG_JA] ? m_btnPdfPrintRangeExpansionMode.frame.origin.y : (m_btnPdfPrintRangeExpansionMode.frame.origin.y - 45);    //国内版と海外版で座標を変える
    
    CGFloat buttonW = [S_LANG isEqualToString:S_LANG_JA] ? m_btnPdfPrintRangeExpansionMode.frame.size.width : (m_btnPdfPrintRangeExpansionMode.frame.size.width + 380);    //国内版と海外版で座標を変える
    
    // iPad用
    m_btnPdfPrintRangeExpansionMode.translatesAutoresizingMaskIntoConstraints = YES;
    m_btnPdfPrintRangeExpansionMode.frame = [self createSpecialButtonCGRectFromY:startY SizeWidth:buttonW SizeHeight:50.0];                        // ボタンサイズ
    ...
    m_btnPdfPrintRangeExpansionMode.titleLabel.font = [UIFont systemFontOfSize:22];                            // ボタンのフォントサイズ
    ...
    if([specialMode boolForKey:S_PDF_PRINT_RANGE_EXPANSION_FLAG])
    {
        m_btnPdfPrintRangeExpansionMode.hidden = NO;
    }
    else
