                                      ...
                                      unsigned long *intp) // Uint to encode
{
  ...
  // build up the header
  data = asn_build_header(data, datalength, type, (int)u_integer_len);
  if (data == NULL)                return NULL;
                   ...
                   unsigned char *data, long dataLength)
{
  ...
  bufPtr = asn_build_string(bufPtr, max_len, ASN_UNI_PRIM | ASN_OCTET_STR,
                            contextEngineID, (int)contextEngineIDLength);
  if (!bufPtr)
  {
  ...
  bufPtr = asn_build_string(bufPtr, max_len, ASN_UNI_PRIM | ASN_OCTET_STR,
                            contextName, (int)contextNameLength);
  if (!bufPtr)
  {
    LOG_BEGIN(ERROR_LOG | 1);
  ...
  outBufPtr = asn_build_sequence(outBufPtr, max_len, ASN_SEQ_CON, (int)bufLength);
  if (!outBufPtr)
  {
    LOG_BEGIN(ERROR_LOG | 1);
