...
@synthesize delegate, parserDelegate, isDownloading, isErr, errCode, errMessage, statusCodeNumber;
...
- (BOOL)getRequest:(NSString *)request;
{
    ...
    //
    NSString *ip = @"";
    ip = [RSCommonUtil getIPAdder];
    NSUInteger len = [[ip stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] length];
    if (len <= 0)
    {
        if (parserDelegate)
        {
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    self.statusCodeNumber = statusCode;
...
- (void)parserDidEndDocument:(NSXMLParser *)parser
{
    if (!isErr)
    {
        ...
        if (parserDelegate)
        {
            [parserDelegate rsManagerDidFinishParsing:self];
        }
    }
    else
    {
...
#pragma mark - 以下のメソッドはサブクラスで実装する
// 以下のメソッドはサブクラスで実装する
