...
#import "TRSGetJobSettableElementsManager.h"
...
@implementation TRSGetJobSettableElementsManager
...
//        blankPageProcessList            = [[NSMutableArray alloc]init];
        ...
        rsSettableElementsData = [[TRSSettableElementsData alloc]init];
    }
    return self;
}
// 解放処理
...
- (void) didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict
{
    ...
    switch (nNest) {
        case E_NEST_RESULT:
            if([self string:elementName isEqualToString:@"result"]) nNest++;
            break;
        case E_NEST_GETJOBSETTABLEELEMENTSRESPONSE:
            if([self string:elementName isEqualToString:@"GetJobSettableElementsResponse"]) nNest++;
            break;
        case E_NEST_XML_DOC_OUT:
            if([self string:elementName isEqualToString:@"xml-doc-out"]) nNest++;
            break;
        case E_NEST_COMPLEX:
            if([self string:elementName isEqualToString:@"complex"]) nNest++;
            break;
        case E_NEST_PROPERTY:
            if([self string:elementName isEqualToString:@"property"])
            {
                ...
                if([self string:sysName isEqualToString:@"ColorMode"]){
                    ...
                    nGetValueStage = E_GET_SPECIALMODE;   // 白紙飛ばし
                }
