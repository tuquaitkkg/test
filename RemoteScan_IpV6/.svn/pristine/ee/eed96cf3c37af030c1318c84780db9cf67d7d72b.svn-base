...
@interface SettingMailServerInfoViewController ()
@property (nonatomic,strong) NSArray *getNumbers;
@property (nonatomic,strong) NSArray *filterSettings;
@end

@implementation SettingMailServerInfoViewController
...
- (void)viewDidLoad
{
    ...
    self.filterSettings = @[obj0,obj1,obj2,obj3];
    ...
    //    self.navigationItem.title = S_SETTING_MAILSERVER;
                             ...
                             MailServerData:(MailServerData*)mailServerData
{
    ...
    DLog(@"%@",[mailServerData description]);
...
- (UIView *) tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    if(section == 1){
        ...
        // タイトルラベル作成
        UILabel* titleLbl = [[UILabel alloc]initWithFrame:CGRectMake(20, 0, tableView.bounds.size.width - 20, 20)];
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    if (textField.tag == 90) {
        ...
        vc.delegate = self;
        vc.navigationItem.title = S_SETTING_MAILSERVER_GET_NUMBER;
        ...
        vc.delegate = self;
        vc.navigationItem.title = S_SETTING_MAILSERVER_FILTER_SETTING;
        [self.navigationController pushViewController:vc animated:YES];
        return NO;
    } else {
        return YES;
